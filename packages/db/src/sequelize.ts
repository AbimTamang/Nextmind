...
/**
 * Env loading is deliberately best-effort and framework-neutral.
 *
 * This package is imported by a Next app, a Nest app and standalone CLI
 * scripts. It previously called @next/env, which made a shared data layer
 * depend on the web framework - the exact coupling the split is meant to
 * remove. Both hosting apps load their own environment before this module is
 * reached; dotenv here only covers the bare `tsx script.ts` case, and silently
 * does nothing when the vars are already present.
 */
